...
- **This repo** — the raw source of truth. Every solved problem gets its own folder with the code and the original problem statement.
- **My other links** — [GitHub](https://github.com/ammar-06) · [LinkedIn](https://www.linkedin.com/in/ammar-ahmad2408/) · [Portfolio](https://ammar-ahmad.vercel.app/)

## 📂 How this repo is organized

Every solved problem lives in its own folder, named `NNNN-problem-slug`, containing:

- the solution file (language varies by problem — Python, Java, TypeScript, JavaScript, SQL, C++, Racket, Erlang, and Elixir all show up here depending on what I felt like using)
- a `README.md` with the original problem statement

The `## LeetCode Topics` section below is auto-generated by LeetHub every time I solve a new problem — it tags every folder by topic (Array, Dynamic Programming, Graph Theory, etc.) so you can jump straight to a category. **This section updates itself; please don't hand-edit it.**

## ⚡ How the web app stays up to date

The web app (`/webapp` in this repo) reads solution data straight from these folders at build time. Because it's deployed on Vercel with this repo connected, **every push automatically triggers a fresh deploy** — and LeetHub pushes a commit here every time I solve a problem on LeetCode. So the pipeline is fully hands-off:

`solve on LeetCode → LeetHub commits → Vercel auto-redeploys → web app is up to date`

No manual steps, no cron jobs.
